...
std::istream& operator>>(std::istream& is, Four& digits) {
    std::string input;
    is >> input;

    for (char c : input) {
        if (c < '0' || c > '3') {
            throw std::invalid_argument("Неверный символ. Ожидаются цифры от 0 до 3.");
        }
    }

    digits.size_ = input.size();
    digits.data_ = new unsigned char[digits.size_];
    
    for (size_t i = 0; i < digits.size_; ++i) {
        digits.data_[i] = input[digits.size_ - 1 - i] - '0';
    }
